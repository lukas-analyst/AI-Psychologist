...
function countPoints() {
  ...
  for (var i = 1; i < otazkyData.length; i++) {
    // Otázka je ve sloupci B a bodování odpovědí ve sloupcích 2, 3, 4, 5, 6 (C, D, E, F, G)
    questionMap[otazkyData[i][1]] = [
      otazkyData[i][2], otazkyData[i][3], otazkyData[i][4], otazkyData[i][5], otazkyData[i][6]
    ];
  }

  var odpovediData = sheetOdpovedi.getDataRange().getValues();
  var odpovediHeader = odpovediData[0];
  var firstQ = 5; // První otázka je ve sloupci F (index 5)
