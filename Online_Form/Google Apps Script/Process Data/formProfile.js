function exportProfile() {
  ...
  for (var i = 1; i < respondentsData.length; i++) {
    ...
    respondentMap[row[0]] = {
      name: row[1],
      gender: row[5], // Gender z Respondents (sloupec F)
      age: Number(row[3]) // Age_calculated z Respondents (sloupec D) - věk vypočítaný k datu odeslání formuláře
    };
  }

  // Kategorie a indexy
  var header = resultsData[0];
  var catOrder = ['IN', 'HY', 'LP', 'EF', 'AG', 'PR', 'GI', 'AN', 'AH', 'CD', 'OD'];
  ...
  function findTScore(gender, age, cat, points) {
    gender = gender.trim().toLowerCase().startsWith('m') ? 'm' : 'z';
  ...
  for (var i = 1; i < resultsData.length; i++) {
    ...
    var id = row[0];
    var respondent = respondentMap[id];
